 ...
 */
/* $Id: gribtonc.c,v 1.14 2002/12/05 20:00:04 rkambic Exp $ */

/* 
 * decodes GRIB products into netCDF files
 * modified May 2002 by Arild Burud/NoSerC, allow user to specify 
 * grib parameter conversion by "-g gribtablefile"
 */
int testNoserc;
...
int nogrow=0; /* grow/no grow unlimited dimension beyond what is defined in the input CDL */
     ...
     char *av0 ; /*  id string */
{
    ...
    (void)fprintf(stderr,
		  "\t-n           Do not grow unlimited dimension beyond CDL define\n") ;
    (void)fprintf(stderr,
    ...
    char *ncname;		/* Pathname of netCDF output file */
{
    ...
    if (ncid == -1) {
	uerror("can't create or size 0 output netCDF file %s, exiting", 
	ncname);
	exit(-1);
    }
     ...
     char *av[] ;
{
	...
	while ((ch = getopt(ac, av, "vxl:t:e:q:g:n")) != EOF) {
	    switch (ch) {
	    case 'v':
		...
		break;
	    case 'n':  /* <<<<< UPC mod 20130505 - grow/no grow unlimited dimension */
		nogrow = 1;
		break;
	    case 't':
		timeo = atoi(optarg) ;
